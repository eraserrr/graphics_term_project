...
public class AI : MonoBehaviour {
	...
	TileManager.Tile GetTargetTilePerGhost()
	{
		...
		case "hulk":	// target = pacman
			...
			break;
		case "spyder":	// target = pacman + 4*pacman's direction (4 steps ahead of pacman)
			...
			targetTile = tiles[manager.Index((int)targetPos.x, (int)targetPos.y)];
			break;
		case "thor":	// target = ambushVector(pacman+2 - blinky) added to pacman+2
			dir = target.GetComponent<PlayerController>().getDir();
			Vector3 blinkyPos = GameObject.Find ("thor").transform.position;
